...
/*****************************************************************************************
	script de chargement du nombre de CESP 2024 dans la table Rang
	depuis la table temporaire CESP (chargée avec un fichier CSV) 
*****************************************************************************************/
	...
	try {
		...
		while ($row = $result->fetch(PDO::FETCH_ASSOC)) {
			...
			if (($CESP != "NULL") and ($CESP != "")) {
				if ($debug) {echo $CodeSpecialite . " - " . $CHU . " - " . $CESP . "\n";}
				$update = "UPDATE Rang SET CESP2024=$CESP WHERE CHU='$CHU' AND CodeSpecialite='$CodeSpecialite';";
		...
		echo "Erreur SELECT CESP : " . $erreur->getMessage();
	}
