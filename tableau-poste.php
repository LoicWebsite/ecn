...
<html lang="fr">
  <head>
    ...
    <title>Nombre de postes d'internat</title>
  ...
  <body id="hautdepage">
	...
	<?php
		...
		// rang du dernier pour l'année en référence
...
//			$where = $where . " AND Lieu = '" . utf8_decode($lieu) . "'";
		...
		echo "<thead><tr>";
		echo "<th>Nombre de postes " . $reference . "<br/><i class='fbi bi-info-circle-fill' data-toggle='tooltip' data-html='true' title='Le nombre de postes d&apos;internat est issu de l&apos;arrêté publié par le Journal Officiel. Ce nombre de postes exclut les CESP.<br/>L&apos;année correspond à l&apos;année de publication au Journal Officiel.'></i></th>";
		...
		foreach ($tableDernier as $CHU) {
			...
			// pour chaque CHU on parcourt les spécialités
			foreach ($CHU as $dernier) {
				...
				// cellule CHU (nom du CHU en première colonne)
				if ($i == 0) {
					...
					if ($rangOk) {
						...
						echo "<td" . $tooltip . ">" . $libelleNbPoste . "</td>";
					}
				}
				...
				$totauxColonnes[$i] += intval($tablePoste[$j][$i]);

				$totalCHU += intval($tablePoste[$j][$i]);
				$grandTotal += intval($tablePoste[$j][$i]);

				$i += 1;
			}

			echo "<td style='background:#eee;'>&nbsp;<strong>" . $montant->format($totalCHU) . "</strong>&nbsp;</td>";
			echo "</tr>";
			$j += 1;
		}
