...
// en attendant que le record avec l'heure de la simulation soit affiché
		...
		echo "Rafraîchir la page <i class='bi bi-arrow-clockwise'></i> pour actualiser.";
		echo "<br/>Double cliquer <i class='bi bi-hand-index-thumb'></i> sur un rang pour afficher le détail CELINE.";
...
// A ACTIVER PENDANT LA PHASE DE CHOIX DE POSTE
					...
					if ($aucuneOffre) {
						$aucuneOffre = false;
						$rangDernier = "";	
						$tooltip = " data-toggle='tooltip' data-html='true' title='" . escapeHtml($CHU) . "<br/>" . escapeHtml($specialite) . "<br/>Aucune offre'";
					} else {
						$tooltip = " data-toggle='tooltip' data-delay='20' data-html='true' title='" . escapeHtml($CHU) . "<br/>" . escapeHtml($specialite) . "<br/>Dernier admis : " . escapeHtml($rangDernier) . "<br/>Postes : " . escapeHtml($offre) . "<br/>Placés : " . escapeHtml($place) . "<br/>Disponibles : " . escapeHtml($dispo) . "'";
					}
...
// fin mise en commentaire temporaire
	...
	<script>
		...
		function questionnaire() {
			<?php
				echo "window.location.href=" . json_encode(buildSafeUrl('questionnaire-choix-specialite.php', ['rang' => $rang, 'reference' => $reference, 'type' => $type, 'lieu' => $lieu, 'internat' => $internat, 'benefice' => $benefice])) . ";";
			?>
		}
		...
		function zoom(code) {
			<?php
				$baseDetailSpecialite = buildSafeUrl('detail-specialite-questionnaire.php', ['rang' => $rang, 'reference' => $reference, 'type' => $type, 'lieu' => $lieu, 'internat' => $internat, 'benefice' => $benefice]);
